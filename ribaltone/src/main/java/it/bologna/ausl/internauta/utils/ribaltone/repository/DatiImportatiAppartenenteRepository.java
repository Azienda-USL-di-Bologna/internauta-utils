...
public interface DatiImportatiAppartenenteRepository extends QuerydslPredicateExecutor<DatiImportatiAppartenente>, JpaRepository<DatiImportatiAppartenente, Integer> {
    ...
    @Query(value = """
        ...
        WHERE  (s.datain < now()  or s.datain is null) AND (s.datafi IS NULL OR s.datafi > now()) 
        AND a.codice_azienda = ?1 
        AND (a.datain < now() or a.datain is null) AND (a.datafi IS NULL OR a.datafi > now()))
        """, nativeQuery = true)
