...
public abstract class JobWorker<T extends JobWorkerData, R extends JobWorkerResult> extends Worker {
    ...
    public UUID calcolaMD5(Boolean future) throws JsonProcessingException, NoSuchAlgorithmException{
     ...
     */
    private String getMD5(JobWorker worker, Boolean future) throws NoSuchAlgorithmException, JsonProcessingException {
        String strData = 
                worker.getName() + 
                (worker.getData() != null? worker.getData().toJsonString(objectMapper): "") +
                worker.isDeferred() +
                (future ? worker.getExecutionTs() : "");
